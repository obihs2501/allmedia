	...
	for i, seg := range segments {
		...
		go func(idx int, seg hlsSegment) {
			...
			if e.opts.Progress != nil {
				e.opts.Progress(0, 0, int(atomic.AddInt32(&segDone, 1)), len(segments))
			}
		}(i, seg)
	}
	wg.Wait()
...
func (e *Engine) downloadDASH(filename string, stream extractor.Stream) (string, error) {
	...
	tmpEngine := &Engine{
		opts:   Opts{Concurrency: e.opts.Concurrency, OutputDir: tmpDir, Overwrite: true, Retries: e.opts.Retries, NoProgress: e.opts.NoProgress, Progress: e.opts.Progress},
		ffmpeg: e.ffmpeg,
		client: e.client,
		http:   e.http,
		ctx:    e.ctx,
