...
type Opts struct {
	...
	// FFmpegLocation, when non-empty, is used as the ffmpeg executable path
	// instead of looking up "ffmpeg" in PATH.
	FFmpegLocation string
	// Progress, when non-nil, receives cumulative download progress.
	// written/total are bytes for byte-oriented downloads; segDone/segTotal
	// are segment counts for HLS segment downloads where byte totals are
	// unknown upfront.
	Progress func(written, total int64, segDone, segTotal int)
}
...
func (e *Engine) downloadSingle(url, outPath string, headers map[string]string, size int64) error {
	...
	if e.opts.Progress != nil {
		w = io.MultiWriter(w, &progressWriter{total: size, report: e.opts.Progress})
	}
...
// progressWriter forwards cumulative byte counts to a Progress callback.
type progressWriter struct {
	written int64
	total   int64
	report  func(written, total int64, segDone, segTotal int)
}

func (p *progressWriter) Write(b []byte) (int, error) {
	p.written += int64(len(b))
	p.report(p.written, p.total, 0, 0)
	return len(b), nil
}
	...
	for i, u := range urls {
		...
		go func(idx int, url string) {
			...
			if info != nil {
				...
				if e.opts.Progress != nil {
					e.opts.Progress(atomic.AddInt64(&progressBytes, info.Size()), totalSize, 0, 0)
				}
			}
		}(i, u)
	}
